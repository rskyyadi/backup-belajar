...
const UbahRealisasi = ({setNavbarTitle}) => {
  ...
  useEffect(() => {
    setNavbarTitle('Ubah Realisasi PPA')
    getInitialData()
  ...
  const FormSection = () => {
    ...
    const formInitialValues = {
      ...
      tgl_ppa_realisasi: dataPPA?.realisasi?.tgl_ppa_realisasi,
    ...
    const initialImageHandler = () => {
      const image = dataPPA?.realisasi?.bukti

      if (image) {
        setImagePreview(process.env.REACT_APP_API_FILE_URL + dataPPA?.realisasi?.bukti)
        setHasImage(true)
      } else {
        setImagePreview('')
        setHasImage(false)
    ...
    useEffect(() => {
      setNoPPA(dataPPA?.realisasi?.no_ppa_realisasi)
      initialImageHandler()
